...
async def test_full_pairing_and_data_round_trip(relay_server_url):
    ...
    assert client_challenge["payload"]["peer_pubkey"] == "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="

    agent_challenge = await agent.recv()
    assert agent_challenge["type"] == "pair_challenge"
    assert agent_challenge["session_id"] == session_id
    assert agent_challenge["payload"]["peer_pubkey"] == "AQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQE="

    await agent.send("pair_complete", session_id=session_id, payload={"agent_proof": "AwMDAwMDAwMDAwMDAwMDAwMDAwMDAwMDAwMDAwMDAwM="})
    complete = await client.recv()
    assert complete["payload"]["agent_proof"] == "AwMDAwMDAwMDAwMDAwMDAwMDAwMDAwMDAwMDAwMDAwM="
...
async def test_second_pair_request_with_same_token_rejected(relay_server_url):
    ...
    token = "tok_" + "f" * 16
    await agent.send("pair_init", payload={"token": token, "agent_pubkey": "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=", "session_id": "sess-dup"})
    ...
    await client2.send(
        "pair_request", payload={"token": token, "client_pubkey": "BAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQ=", "client_proof": "BQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQU="}
    )
...
async def test_pair_request_unknown_token_rejected(relay_server_url):
    ...
    await client.send(
        "pair_request",
        payload={"token": "tok_" + "z" * 16, "client_pubkey": "AQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQE=", "client_proof": "AgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgI="},
    )
...
async def test_connection_cannot_route_to_another_session(relay_server_url):
    agent = await FakePeer.connect(relay_server_url, "agent")
    client = await FakePeer.connect(relay_server_url, "client")
    token = "tok_" + "y" * 16
    await agent.send(
        "pair_init", payload={"token": token, "agent_pubkey": "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=", "session_id": "sess-bound"}
    )
    await agent.recv()
    await client.send(
        "pair_request", payload={"token": token, "client_pubkey": "AQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQE=", "client_proof": "AgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgI="}
    ...
    await client.send("pty_input", session_id="sess-someone-else", payload={"nonce": "n", "ciphertext": "c"})
